...
> **Lingkungan sesi 2026-09-05 (sesi 5):** `backend/.env` WAJIB `CORS_ORIGINS` eksplisit
> (mis. `https://<preview>.preview.emergentagent.com,http://localhost:3000`) — nilai `*` membuat
> backend menolak start. Frontend dilayani dari `frontend/build` (TANPA hot reload):
> `bash scripts/rebuild_frontend.sh` sesudah mengubah `frontend/src`.
