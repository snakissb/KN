...
## REGISTRY — SESI 2026-06 (lanjutan ke-4)

### KN-2606-COST-BACKFILL — migrasi startup menghidupkan HPP nol tanpa jurnal (akar INV-GL-DRIFT)
- Tanggal       : 2026-06 (sesi lanjutan ke-4)
- Modul         : backend/bootstrap.py (`backfill_costing_data`)
- Gejala        : WARN `verify_data_integrity` GL-3 `ent_kanda(Δ900.000)` yang SELALU kembali
                  walau sudah di-true-up. Roll retur pelanggan `RTN-00001` berkondisi
                  `damaged` (nilai sengaja Rp 0, HPP-nya sudah dibalik nota kredit) diisi
                  ulang menjadi Rp 90.000/unit dari `products.harga_pokok` SETIAP kali
                  backend hidup, tanpa satu pun jurnal `1-1300`. Tidak ada galat: subledger
                  naik, GL tidak.
- Perbaikan     : "belum pernah dinilai" = field `base_unit_cost` DAN `unit_cost` yang MEMANG
                  TIDAK ADA (bukan bernilai 0). Roll yang nilainya keputusan orang
                  (`cost_basis` / `writeoff_je_id` / `condition=damaged`) tidak pernah disentuh.
- Invariant     : INV-GL-DRIFT
- Gate penangkap: verify_data_integrity GL-3 (WARN → PASS) ·
                  backend/test_core_sesi_2026_06b_poc.py H1b (roll Rp 0 tetap Rp 0 sesudah
                  migrasi, DAN roll tanpa nilai tetap diisi)
- Severity      : P1
- Status        : FIXED (bukti: GL-3 PASS · POC 35/0 · gate --full HIJAU 402 s)

### KN-2606-ICRET-COST-READ — jurnal barang retur antar-PT dilewati karena rumus nilai berbeda
- Tanggal       : 2026-06 (sesi lanjutan ke-4)
- Modul         : services/interco_return_service.py (`on_return_task_executed`)
- Gejala        : roll ber-`unit_cost` 0 tetapi `base_unit_cost` masih bernilai menghasilkan
                  `cost_back`/`carry_out` = 0, sehingga `interco_return:<pair>:goods_in` &
                  `:goods_out` TIDAK diposting padahal barangnya benar-benar berpindah gudang.
- Perbaikan     : nilai roll dibaca `unit_cost or base_unit_cost` — rumus yang SAMA dengan
                  `inventory_reconciliation`, jadi "barang masuk" mustahil tak berjurnal.
- Invariant     : INV-GL-DRIFT · INV-IC-08
- Gate penangkap: backend/test_core_sesi_2026_06b_poc.py H1
- Severity      : P1
- Status        : FIXED (bukti: POC 35/0 · dua jurnal terbit · drift kembali 0)

## REGISTRY — SESI 2026-06 (lanjutan ke-5)

### KN-2606-COST-NO-TRAIL — HPP roll bisa berubah dari 4 arah tanpa satu pun jejak
- Tanggal       : 2026-06 (sesi lanjutan ke-5)
- Modul         : bootstrap.backfill_costing_data · interco_return_service ·
                  interco_service · landed_cost_service
- Gejala        : nilai persediaan sebuah roll bisa naik/turun (mis. Rp 0 → Rp 90.000)
                  tanpa cara apa pun untuk mengetahui SIAPA & atas dasar apa. Inilah yang
                  membuat `KN-2606-COST-BACKFILL` bertahan berbulan-bulan: gejalanya
                  (selisih GL vs subledger) terlihat, penyebabnya tidak.
- Perbaikan     : `services/roll_cost_history.py` + koleksi `roll_cost_history`; SEMUA
                  penulis HPP roll memanggil `record()`. Dibaca
                  `GET /api/inventory/rolls/{id}/cost-history` → panel detail roll +
                  bukti pada tuduhan selisih.
- Invariant     : INV-GL-DRIFT (pendukung) · INV-ENT-01 (koleksi ber-entity_id wajib
                  terdaftar di `entity_scope.SCOPED_COLLECTIONS`)
- Gate penangkap: POC E-0 L15 (MEMERAH saat koleksi baru belum terdaftar — terjadi nyata
                  di sesi ini) · test_core_sesi_2026_06c_poc.py N2
- Severity      : P2
- Status        : FIXED (bukti: POC 21/0 · E-0 83/0 · gate --full HIJAU 395 s)

### KN-2606-POC-RESIDU-ELIM — POC menyisakan entri eliminasi konsolidasi tak seimbang
- Tanggal       : 2026-06 (sesi lanjutan ke-5)
- Modul         : backend/test_core_sesi_2026_06b_poc.py · ..._06c_poc.py (snapshot POC)
- Gejala        : sesudah POC dijalankan, `verify_data_integrity` INV-IC-03 memerah
                  ("entri eliminasi tidak seimbang") — dokumen retur POC memicu
                  perhitungan ulang eliminasi pada pair demo NYATA, dan koleksi
                  `intercompany_eliminations` tidak ikut disnapshot.
- Perbaikan     : snapshot POC kini mencakup `journal_entries`, `interco_accounts`,
                  `intercompany_eliminations`, `purchase_orders`.
- Invariant     : INV-GATE-01 (POC nol residu)
- Gate penangkap: verify_data_integrity (dijalankan SETELAH POC, tidak bersamaan)
- Severity      : P2
- Status        : FIXED (bukti: integritas 242/0 pasca-POC)
