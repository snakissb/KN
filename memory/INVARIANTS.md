...
| **INV-ATOMIC-01** | Endpoint tulis ≥2 koleksi substantif WAJIB (a) `services/atomic_claim.claim()` SESUDAH validasi 400 & SEBELUM tulisan turunan, tulisan akhir mencabut `saga_lock` (`finish_set`/`$unset`/`so_transition`), ATAU (b) CAS: `find_one_and_update` induk berprasyarat `status` + turunan idempoten, ATAU (c) alasan ≥20 huruf di `REVIEWED`. "BELUM DITINJAU" RATCHET (`BASELINE_UNREVIEWED` hanya turun). Kunci menggantung TERLIHAT di `GET /api/saga-locks` & bisa dilepas admin — bukan menulis dua kali diam-diam | T-01 audit 2026-09 (keputusan Opsi B — saga, tanpa replica set): resolve-escalation melepas roll 2× saat ulang-jalan; 87 endpoint multi-koleksi tanpa penjaga | `scripts/guardrails/verify_atomic_claim.py` | STATIK | `REVIEWED` di skrip (mekanisme + alasan per endpoint) |
