...
PUBLIC_ALLOWLIST = {
    ...
    # Middleware RFID (gate/handheld/printer) — auth via header X-Device-Key
    # (`rfid_ingest_service.authenticate` → 401 bila kosong/tak dikenal), bukan sesi.
    "POST /rfid/ingest",
    "POST /rfid/heartbeat",
    "GET /rfid/device-jobs/pending",
    "POST /rfid/device-jobs/{job_id}/ack",
}
