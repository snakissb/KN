...
> Diperbarui: **2026-06 (sesi lanjutan ke-8)** — repo kini di
> `github.com/pandeyoga/KN020926`; lingkungan di-restore dengan `.restore_env.sh`
> (mongo hidup → restart backend → seed → fondasi lengkap → build FE), data demo
> persis (makloon 5 · inspeksi 3 · vendor_bills 12 · produk 20). **3 temuan KRITIS
> `TEMUAN_AUDIT_TRAINING.md` DIPERBAIKI:**
> * **T1** — `sales_admin.require_verification_before_confirm` bawaan **HIDUP**
>   (`config_catalog_core.py`), dan `confirm_order` menolak 409 "belum disetujui
>   manajer" bila status ≠ `approved` & (`approval_required` atau approval
>   menggantung). Urutan gerbang = dokumen training: verifikasi → ACC manajer →
>   konfirmasi. FE `SalesAdminDesk.doConfirm` hanya menawarkan "Verifikasi
>   sekarang" bila galatnya memang soal verifikasi. POC `test_core_e8_desk_poc.py`
>   disesuaikan (cek "bawaan HIDUP", lalu matikan → tidak tertahan → nyalakan lagi).
> * **T2** — `routers/internal_requests.py`: `sales_admin` masuk `DECIDER_ROLES` &
>   `CROSS_ENTITY_ROLES` → Meja Admin Sales "Tindak" tidak lagi buntu (list/
>   sources/convert 200). Sales tetap hanya-miliknya.
> * **T3** — `entity_scope.assert_active_entity_access` (baru; detail seketat daftar:
>   badan usaha AKTIF, mode `all` → pagar penugasan) dipakai `inspections._guarded`.
> Bukti: POC E-8 desk **97/97** · `gate.sh --full` **HIJAU** · iter275 A4 & Alur F
> PASS (sisa merah = D1 data demo & T4) · iter276 DD2 manager=**404**.
> **BERIKUTNYA:** T4–T5 (TINGGI), T6–T9, T10–T11, paket seed D1–D5.
...
> Atas permintaan pemilik **tidak ada satu baris pun diperbaiki**; dokumen itu memuat
> bukti tiap temuan, kenapa ia lolos semua pagar, dan usul urutan pengerjaan
> (T1 → T3 → T2 → T4 → T5).
> Paling perlu diketahui: **T1** — endpoint riwayat nilai roll memakai izin `wms.view`,
> sehingga peran **finance** (yang layarnya justru memakainya) dijawab 403 dan galatnya
> DITELAN `try/catch` → blok bukti riwayat nilai tidak pernah muncul di Rekonsiliasi
> Persediaan tanpa satu pun pesan.

> Diperbarui: **2026-06 (sesi lanjutan ke-5)** — **NILAI TAGIHAN SUPPLIER di papan ·
> RIWAYAT NILAI (HPP) ROLL · PAPAN BISA DITINDAK**. `gate.sh --full` HIJAU 395 s ·
> `verify_data_integrity` **242 PASS · 0 FAIL · 0 WARN** · POC baru
> `test_core_sesi_2026_06c_poc.py` **21 PASS · 0 FAIL** (nol residu) · POC `2026-06b`
> **35 PASS** · POC isolasi E-0 **83 PASS** (pagar baru: koleksi `roll_cost_history`
> terdaftar di registry badan usaha) · agen uji `iteration_254` **backend 8/8 ·
> frontend alur act→konfirmasi→toast→muat-ulang terbukti**.

## SESI 2026-06 (lanjutan ke-5) — UANG YANG TERTAHAN TERLIHAT · NILAI ROLL BERJEJAK · PAPAN BISA DITINDAK

### 1. Nilai rupiah di papan tagihan supplier & kontrabon
`DETAIL_META` mendapat `vendor_bill` (`grand_total`/`net_amount`, keterangan = nomor
faktur supplier) serta `contra_bon_verify`/`contra_bon_approve`
(`totals.net_payable`). Papan yang menahan UANG kini menyebut besarnya tanpa perlu
membuka layar. Field diperiksa dari dokumen NYATA (INV-AGING-01), bukan ditebak.

### 2. Riwayat nilai (HPP) roll — `services/roll_cost_history.py` (koleksi `roll_cost_history`)
HPP satu roll bisa berubah dari EMPAT arah dan dulu tak satu pun berjejak: migrasi
startup, revaluasi retur antar-PT, revaluasi pembelian antar-PT, dan alokasi voucher
biaya masuk. Semua pemanggil sekarang menulis `record()` (nilai lama → baru,
`delta_value`, ALASAN dari daftar tertutup, aktor, dokumen sumber). Dibaca:
`GET /api/inventory/rolls/{id}/cost-history` (403 di luar penugasan) → panel
**Riwayat Nilai (HPP)** di `RollGradePanel`, dan ikut ditempel sebagai bukti pada
tuduhan selisih persediaan (`recon-suspect-cost-history`).
Pagar: koleksinya WAJIB terdaftar di `entity_scope.SCOPED_COLLECTIONS` — POC E-0 L15
memerah saat belum terdaftar (dan itu benar-benar terjadi di sesi ini).

### 3. Papan bisa ditindak (`ACTION_META`)
Baris papan membawa `action` = pintu keputusan yang SUDAH ada di router (path disusun
SERVER, mis. SO menunjuk entri `pending_approvals` yang menunggu). Tombolnya hanya
diberikan bila peran pemanggil berwenang (`routers/home.py::_allowed` dari
`permission_matrix`) — layar tidak pernah menebak wewenang, dan endpoint tujuannya
tetap memeriksa sendiri (pemisahan tugas, ambang nilai, status). Konfirmasi wajib,
catatan opsional (`askReason` dengan `reasonRequired:false`).
SENGAJA tanpa tombol (alasan ditulis di `ACTION_META`): `contra_bon_dispute` &
`payment_variance` (keputusannya PILIHAN, bukan "setujui"), `inspection_hold`
(pelepasan per BARIS + alasan wajib), `rnd_sample`.

### 4. Catatan pengujian (untuk agen berikutnya)
POC memakai snapshot/restore basis data. **Jangan** menjalankan
`verify_data_integrity` atau gate BERSAMAAN dengan POC apa pun: pemeriksa lain akan
melihat keadaan sementara dan memerah atas alasan yang salah (terjadi dua kali di sesi
ini). POC `2026-06b`/`2026-06c` kini ikut menyalin `journal_entries`,
`interco_accounts`, `intercompany_eliminations`, dan `purchase_orders` supaya
nol-residunya benar-benar nol.

---

> Diperbarui: **2026-06 (sesi lanjutan ke-4)** — **SELISIH KANDA LUNAS DI AKARNYA**
> (migrasi startup yang menghidupkan HPP nol + jembatan jurnal retur antar-PT),
> **tuduhan selisih bisa diklik**, **lonceng berhalaman**, **Papan Keuangan di Meja
> Finance**. `gate.sh --full` HIJAU 402 s · `verify_data_integrity` GL-3 kini **PASS**
> (dulu WARN `ent_kanda Δ900.000`) · POC baru `test_core_sesi_2026_06b_poc.py`
> **35 PASS · 0 FAIL** (nol residu) · agen uji `iteration_253` backend & frontend 100%.
> Sebelumnya: 2026-08-24 (lanjutan ke-3) — PAPAN PO CUSTOM di beranda pemilik.

## SESI 2026-06 (lanjutan ke-4) — AKAR INV-GL-DRIFT DITUTUP + TIGA PEKERJAAN LAYAR

### 1. `INV-GL-DRIFT` (selisih Rp 900.000 buku CV Kanda Suka) — **LUNAS**
Ada DUA cacat, keduanya ditutup:

1. **Akarnya bukan transaksi, melainkan migrasi startup.**
   `bootstrap.backfill_costing_data` menganggap `base_unit_cost == 0` sebagai "data
   cost hilang" dan MENGISINYA dari `products.harga_pokok` **setiap kali backend
   hidup**. Nol yang SENGAJA — roll retur pelanggan `damaged`, roll yang sudah
   dihapus-buku, roll yang dinilai ulang retur antar-PT — karena itu dihidupkan lagi
   menjadi Rp 90.000/unit **tanpa satu pun jurnal 1-1300**. Itu sebabnya setiap
   true-up hanya menutupi selisihnya sampai backend hidup berikutnya. Sekarang yang
   dianggap "belum pernah dinilai" hanya field yang MEMANG TIDAK ADA, dan roll yang
   nilainya keputusan orang (`cost_basis` / `writeoff_*` / `condition=damaged`) tidak
   pernah disentuh.
2. **Jembatan retur antar-PT membaca nilai roll dengan rumus yang berbeda dari
   rekonsiliasi.** `interco_return_service.on_return_task_executed` hanya membaca
   `unit_cost`, sementara rekonsiliasi memakai `unit_cost or base_unit_cost`. Roll
   ber-`unit_cost` 0 tetapi `base_unit_cost` masih bernilai menghasilkan
   `cost_back`/`carry_out` = 0 → jurnal `interco_return:…:goods_in` DILEWATI padahal
   barangnya benar-benar masuk gudang penjual. Kini keduanya memakai rumus yang sama,
   jadi "barang masuk" selalu berjurnal di kedua buku.

### 2. Tuduhan selisih **bisa diklik** (`suspects[].ref`)
`gl_service.inventory_drift_explain` kini melampirkan ALAMAT pada tiap tuduhan
(`_roll_ref` → roll id/nomor + dokumen asalnya, jurnal true-up terakhir, atau akun
1-1300). Layar Rekonsiliasi Persediaan membuka **panel bukti** di tempat
(`recon-suspect-open-{kind}` → `recon-suspect-evidence-{kind}`): roll (nomor · gudang ·
HPP · nilai · dokumen asal) atau jurnal (nomor · tanggal · baris debit/kredit).
Sebelum ini kalimat "Roll RTN-00001 bernilai Rp 900.000" berhenti sebagai TEKS.

### 3. Lonceng **berhalaman**
`GET /api/notifications?page=&page_size=` menjawab envelope
`{items,total,page,page_size,has_more}` (tanpa parameter tetap array telanjang —
kompatibel mundur). Kotak notifikasi punya `notif-load-more` + penanda akhir
`notif-list-end` ("X dari Y notifikasi"), jadi angka lencana selalu bisa dibuktikan
dari daftar. Isolasi TIDAK dilonggarkan (`_scope_query` tak berubah).

### 4. **Papan Keuangan** di Meja Finance
`FINANCE_BOARD_KEYS = (contra_bon_approve, contra_bon_verify, contra_bon_dispute,
vendor_bill)` + `GET /api/home/finance` (403 untuk badan usaha di luar penugasan).
Angka & umur tunggu tetap milik `approval_backlog_service` (INV-HOME-01), jadi papan
mustahil berselisih dari Pusat Persetujuan. Layar: `WaitingBoardsStrip` di FinanceDesk
(`fin-home-board-{key}`).

### 5. Verifikasi
`gate.sh --full` HIJAU **402 s** · `verify_data_integrity` **PASS 242 · FAIL 0 · WARN 0**
(GL-3 rekonsiliasi persediaan kini PASS) · POC `test_core_sesi_2026_06b_poc.py`
**35 PASS · 0 FAIL** · POC sesi 2026-06 **66 PASS** · POC isolasi E-0 **83 PASS** ·
agen uji `iteration_253` **backend 100% · frontend 100%, nol temuan**.
