...
export default function NotificationCenter({
  ...
  total = 0, hasMore = false, onLoadMore,
      ...
      {open && (
        <div className="notif-panel" data-testid="notif-panel">
          ...
          <div className="notif-filters" data-testid="notif-filters">
            ...
            <span className="notif-filter-count" data-testid="notif-filter-count">
              ...
              {total > notifications.length && (
                <span data-testid="notif-total-hint">
                  {" "}(dari {total} total)
                </span>
              )}
              {hasFilter && (
          ...
          <div className="notif-list" data-testid="notif-list">
            ...
            {/* LONCENG BERHALAMAN (2026-06) — daftar bisa ditelusuri sampai baris
                terakhir, jadi angka lencana selalu bisa dibuktikan dari layar. */}
            {hasMore && (
              <button type="button" data-testid="notif-load-more"
                ...
                {loadingMore ? "Memuat…"
                  : `Muat lebih banyak (${Math.max(0, total - notifications.length)} lagi)`}
              </button>
            )}
            {!hasMore && notifications.length > 0 && total > 0 && (
              <p className="notif-list-end" data-testid="notif-list-end">
                Sudah sampai baris terakhir · {notifications.length} dari {total} notifikasi
              </p>
            )}
          </div>
        </div>
