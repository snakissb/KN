...
import { ClipboardList, ScanLine, Tags, PackageCheck, AlertTriangle, CheckCircle2, Loader2 } from "lucide-react";
...
function TaskList({ type }) {
  ...
  const load = () => axios.get(`${API}/wms/tasks`).then((r) => setRows((Array.isArray(r.data) ? r.data : r.data.items || r.data.tasks || []).filter((t) => (t.flow_type || t.type) === type && !["completed", "shipped", "cancelled"].includes(t.status))))
  ...
  if (!rows.length) return <div className="p-8 text-center text-sm text-[#6E6E73]" data-testid="mw-task-empty">Tidak ada tugas {type === "inbound" ? "barang masuk" : "barang keluar"} yang terbuka.</div>;
  return (
    <div className="space-y-2 p-3" data-testid={`mw-task-list-${type}`}>
      {rows.map((t) => (
        <div key={t.id} className="m-card p-3" data-testid={`mw-task-${t.id}`}>
          ...
          <div className="text-xs text-[#6E6E73] mt-1">{t.warehouse_name || t.warehouse_id} · qty <b className="tabular-nums">{t.quantity}</b> {t.unit || ""}{t.order_number ? ` · ${t.order_number}` : ""}{t.po_number ? ` · ${t.po_number}` : ""}</div>
        </div>
      ))}
    </div>
  );
