...
import { AlertTriangle, Coins, History, ShieldCheck, X } from "lucide-react";
...
export default function RollGradePanel({ rollId, rollNo = "", currentUser, onClose, onChanged }) {
  ...
  const fmtCur = (v) => `Rp ${new Intl.NumberFormat("id-ID").format(Math.round(v || 0))}`;

  const [data, setData] = useState(null);
  const [costs, setCosts] = useState(null);   // riwayat NILAI (HPP) roll — 2026-06
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [g, c] = await Promise.all([
        axios.get(`${API}/inventory/rolls/${rollId}/grade-history`),
        axios.get(`${API}/inventory/rolls/${rollId}/cost-history`).catch(() => ({ data: null })),
      ]);
      setData(g.data || null);
      setGrade(g.data?.grade || "");
    ...
    <div className="modal-overlay" data-testid="roll-grade-panel">
      <div className="modal-card max-w-[560px]">
        ...
        <div className="mt-2 grid gap-3">
          {/* RIWAYAT NILAI (2026-06) — HPP roll bisa berubah dari beberapa arah
              (migrasi startup · revaluasi retur/pembelian antar-PT · biaya masuk).
              Selama perubahan itu tak berjejak, kenaikan nilai persediaan bisa
              terjadi diam-diam — itulah yang membuat selisih Rp 900.000 di buku
              CV Kanda Suka bertahan berbulan-bulan. */}
          <div className="rounded-md border border-[#DCE6F7] bg-[#F6F9FF] p-2.5"
            data-testid="roll-cost-history">
            <p className="mb-1.5 flex items-center gap-1 text-[11px] font-bold uppercase tracking-wide text-[#33538B]">
              <Coins size={12} /> Riwayat Nilai (HPP)
            </p>
            <p className="mb-1.5 text-[11.5px] text-[#3C3C43]">
              HPP kini <b data-testid="roll-cost-current">{fmtCur(costs?.unit_cost)}</b>/unit ·
              sisa {costs?.length_remaining ?? "—"} unit ·
              nilai <b>{fmtCur((costs?.unit_cost || 0) * (costs?.length_remaining || 0))}</b>
            </p>
            {(costs?.history || []).length === 0 ? (
              <p className="text-[11.5px] text-[#6B6B73]" data-testid="roll-cost-history-empty">
                Belum ada perubahan HPP tercatat untuk roll ini.
              </p>
            ) : (
              <div className="grid gap-1.5">
                {(costs.history || []).map((h, i) => (
                  <div key={h.id || i} data-testid={`roll-cost-history-${i}`}
                    className="rounded-md border border-[#EFF0F2] bg-white px-2 py-1.5 text-[11.5px]">
                    <div className="flex flex-wrap items-center gap-1.5">
                      <span className="font-mono font-semibold">
                        {fmtCur(h.old_unit_cost)} → {fmtCur(h.new_unit_cost)}
                      </span>
                      <span className={`status-pill ${h.direction === "turun" ? "pill-danger" : "pill-success"}`}>
                        {h.direction} {fmtCur(Math.abs(h.delta_value))}
                      </span>
                    ...
                    <p className="mt-0.5 text-[10.5px] text-[#3C3C43]">
                      {h.reason_label || h.reason} · oleh <b>{h.actor || "sistem"}</b>
                      {h.ref_number ? ` · dokumen ${h.ref_number}` : ""}
                      {h.note ? ` · ${h.note}` : ""}
                    </p>
                  </div>
                ))}
