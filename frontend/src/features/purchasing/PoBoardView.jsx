...
export default function PoBoardView({ selectedEntity = "all", onOpenDocument }) {
              ...
              <div className="divide-y divide-[#EFF0F2]">
                {rows.map((r) => (
                  <div key={r.po_id} data-testid={`po-board-row-${r.po_id}`}
                    ...
                    <div className="min-w-0">
                      {/* Klik nomor PO → buka dokumen PO di layar Pesanan Pembelian */}
                      <button type="button" data-testid={`po-board-open-${r.po_id}`}
                        title="Buka dokumen PO"
                        onClick={() => onOpenDocument?.({ nav_id: "purchase-orders", view: "purchasing", focus_type: "purchase_order", focus_id: r.po_id })}
                        className="block max-w-full truncate text-left text-[11.5px] font-bold text-[#0058CC] hover:underline">
                        {r.po_number}
                      </button>
                      {r.pr_number ? (
