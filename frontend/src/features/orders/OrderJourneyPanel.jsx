...
export default function OrderJourneyPanel({ orderId, orderNumber }) {
  ...
  const logistik = Array.isArray(data?.logistics) ? data.logistics : [];   // FB-02
    ...
    <aside className="section-card" data-testid="order-journey-panel">
      ...
      {loading && !data ? (
        ...
        <>
          ...
          <Block title="Pelacakan Pengiriman (Logistik)" icon={Truck} testId="journey-logistics"
                 empty="Belum ada pengiriman logistik — Surat Jalan belum diangkut." rows={logistik}
                 render={(l) => (
                   <>
                     <span className="flex items-center gap-1.5 text-[11.5px] font-semibold">
                       <span className="font-mono text-[#0058CC]">{l.number}</span>
                       <span className={`status-pill ${l.status === "delivered" || l.status === "completed" ? "pill-success" : l.status === "failed" ? "pill-danger" : "pill-warning"}`}
                             data-testid={`journey-logistics-status-${l.id}`}>{l.status_label}</span>
                     </span>
                     <span className="text-[10.5px] text-[#8E8E93]">
                       {l.mode === "expedition" ? `${l.courier_name || "Ekspedisi"} · resi ${l.tracking_no || "—"}` : `Armada sendiri · ${l.vehicle_plate || "—"} · ${l.driver_name || "—"}`}
                       {l.eta ? ` · ETA ${l.eta}` : ""}
                     </span>
                     {l.last_position && (
                       <span className="text-[10.5px] text-[#6B6B73]">Posisi terakhir: <b>{l.last_position.location}</b> ({String(l.last_position.at).slice(0, 16).replace("T", " ")})</span>
                     )}
                     {l.pod && <span className="text-[10.5px] text-[#1B7F4B]">Diterima {l.pod.receiver_name}</span>}
                     <button type="button" data-testid={`journey-logistics-open-${l.id}`} onClick={() => openLogistics({ deliveryId: l.id })}
                       className="self-start mt-1 inline-flex items-center gap-1 text-[10.5px] font-semibold text-[#0058CC] hover:underline">
                       Buka di Logistik (foto muat/POD, peta posisi) →
                     </button>
                   </>
                 )} />
