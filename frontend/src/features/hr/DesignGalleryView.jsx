...
import { ACCEPT_IMG, MAX_IMG_MB, validateImage, fmtBytes } from "./galleryUtils";
import GalleryImage from "./GalleryImage";
import AiIllustrationPanel from "./AiIllustrationPanel";

// FB-01 — berkas `kind: ai_illustration` = ilustrasi arahan AI, bukan artwork.
const isArtwork = (f) => (f.kind || "artwork") !== "ai_illustration";
...
function GalleryCard({ g, canManage, onManage, onDelete }) {
  const cover = (g.files || []).find(isArtwork);
  const aiIllus = (g.files || []).filter((f) => !isArtwork(f)).length;
  ...
  return (
    <div className="section-card !p-0 overflow-hidden flex flex-col" data-testid={`gallery-card-${g.id}`}>
      <div className="aspect-[4/3] bg-[#F2F3F5] relative">
        ...
        {aiIllus > 0 && <span data-testid={`gallery-card-ai-illus-${g.id}`} title="Ada ilustrasi arahan AI (bukan artwork)" className="absolute bottom-2 right-2 px-1.5 py-0.5 rounded bg-[#5B2EA6] text-white text-[10px] font-semibold flex items-center gap-1"><Wand2 size={10} /> {aiIllus} ilustrasi AI</span>}
      </div>
    ...
    <div className="modal-overlay" data-testid="gallery-manage-modal" onClick={(e) => { if (e.target === e.currentTarget && !busy && !uploadBusy && !aiBusy) onClose(); }}>
      <div className="modal-card !max-w-[640px]">
        ...
        <div className="grid gap-1.5 mt-2">
          <label className="text-[11px] font-bold uppercase text-[#6B6B73]">Artwork ({files.length})</label>
