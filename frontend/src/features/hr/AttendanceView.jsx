...
import KNTimePicker from "@/components/KNTimePicker";
          ...
          <div className="section-card mb-3">
            ...
            {showManual && canManage && (
              <div className="section-body border-t border-[#EFF0F2] grid gap-2 md:grid-cols-[1.4fr_130px_110px_110px_140px_auto]" data-testid="attendance-manual-panel">
                ...
                <KNDatePicker data-testid="manual-date" value={manual.date} onChange={(v) => setManual((m) => ({ ...m, date: v }))} />
                <KNTimePicker data-testid="manual-in" placeholder="Jam masuk" value={manual.clock_in} onChange={(v) => setManual((m) => ({ ...m, clock_in: v }))} />
                <KNTimePicker data-testid="manual-out" placeholder="Jam keluar" value={manual.clock_out} onChange={(v) => setManual((m) => ({ ...m, clock_out: v }))} />
        ...
        <>
          <div className="section-card mb-3"><div className="section-body flex items-end gap-2">
            <div><label className="block text-[10.5px] font-semibold text-[#6B6B73] mb-1">Periode (Bulan)</label>
              <div className="w-[190px]"><KNMonthPicker data-testid="recap-month" value={month} onChange={setMonth} /></div></div>
          </div></div>
          {loadingRecap ? (
