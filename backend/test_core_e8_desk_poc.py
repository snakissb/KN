...
def e_verify(sa, adm, s1):
    ...
    # T1 audit training 2026-06: bawaan kini HIDUP (dokumen training menjanjikan gerbang).
    check(f"bawaan sakelar `{CFG_VERIFY}` = HIDUP (T1 audit training)",
          _cfg_value(adm, CFG_VERIFY) in (True, "true", 1), str(_cfg_value(adm, CFG_VERIFY)))
    if kandidat:
        blocked = sa.post(f"/api/sales-orders/{kandidat['ref_id']}/confirm")
        check("bawaan HIDUP: konfirmasi pesanan belum terverifikasi DITOLAK 409",
        ...
        r_off = cfg_set(adm, CFG_VERIFY, False)
        check("sakelar bisa dimatikan dari Pusat Pengaturan", r_off.status_code == 200,
              f"HTTP {r_off.status_code}")
        check("setelah sakelar dimatikan, konfirmasi tidak lagi tertahan gerbang verifikasi",
              sa.post(f"/api/sales-orders/{kandidat['ref_id']}/confirm").status_code != 409)
        cfg_set(adm, CFG_VERIFY, True)
    else:
