...
def g6_backlog_satu_pintu(cl):
    ...
    # Sesi 4 (2026-09): finance TIDAK punya `approval.view`/`order.approve` (SoD: sisi uang
    # masuk & pajak, bukan pemutus) → 403. Asersi lama "finance 200" adalah uji basi.
    boleh = ("admin", "manager", "sales_admin")   # `approval.view`/`order.approve`
    tolak = ("sales", "warehouse", "finance")     # bukan pemutus/pengejar approval
    for role in boleh:
