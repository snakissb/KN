...
from pagination import is_paged, get_page_params, paginate_list

router = APIRouter(prefix="/api")

# T-03 Lapis 4 — hanya field yang dipakai Kartu Asal; roll penuh (scan_log, cost history,
# refs) tidak lagi ikut terangkut dari Mongo.
_HISTORY_PROJECTION = {
    "_id": 0, "id": 1, "roll_no": 1, "lot": 1, "status": 1, "grade": 1, "unit": 1,
    "po_id": 1, "po_number": 1, "acquired": 1, "supplier_id": 1, "supplier_name": 1,
    "vendor_bill_id": 1, "supplier_invoice_no": 1, "received_date": 1, "created_at": 1,
    "length_initial": 1, "length_remaining": 1, "unit_cost": 1, "base_unit_cost": 1,
    "landed_cost_total": 1,
}
...
async def product_purchase_history(
    ...
    rolls = await db.inventory_rolls.find(q, _HISTORY_PROJECTION).to_list(20000)
    ...
    # T-03 Lapis 4 — opt-in paginasi event (`?page=&page_size=`): ringkasan tetap utuh,
    # daftar event dipotong per halaman + `events_page` berisi envelope.
    if is_paged(request):
        page, page_size, _, _ = get_page_params(request, default_size=20)
        paged = paginate_list(events, page, page_size)
        out["events"] = paged["items"]
        out["events_page"] = {k: v for k, v in paged.items() if k != "items"}
    return out
