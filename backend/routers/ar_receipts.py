...
async def _perm_ar_receipt_create(request: Request) -> Dict[str, Any]:
    # T-10 (audit 2026-09) — izin dicek sebagai DEPENDENCY supaya berjalan SEBELUM
    # validasi body: pemanggil tak berwenang menerima 403, bukan 422 berisi bentuk skema.
...
async def create_receipt(payload: ReceiptPayload, request: Request,
                         actor: Dict[str, Any] = Depends(_perm_ar_receipt_create)) -> Dict[str, Any]:
    ctx = await entity_ctx(request)
