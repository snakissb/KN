...
async def track_history(request: Request, employee_id: str, date: str = None,
    ...
    T-03 Lapis 4 — opt-in paginasi (`?page=&page_size=`) → envelope; tanpa param → array."""
    await require_permission(request, "hr", "view")
    ctx = await entity_ctx(request)
    ...
    if is_paged(request):
        page, page_size, _, _ = get_page_params(request, default_size=200, max_size=1000)
        items, total = await fetch_page(db.hr_field_tracks, q, page, page_size, sort_field="ts", sort_dir=1)
...
async def list_visits(request: Request, entity_id: str = None, date_from: str = None,
    ...
    if is_paged(request):
        page, page_size, _, _ = get_page_params(request, default_size=50)
        items, total = await fetch_page(db.hr_visits, q, page, page_size, sort_field="created_at", sort_dir=-1)
        return envelope(items, total, page, page_size)
