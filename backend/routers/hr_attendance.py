...
async def list_attendance(request: Request, entity_id: str = None, date_from: str = None,
    ...
    T-03 Lapis 4 — opt-in paginasi (`?page=&page_size=`) → envelope; tanpa param → array (kompatibel)."""
    await require_permission(request, "hr", "view")
    ctx = await entity_ctx(request)
    q: Dict[str, Any] = {}
    if employee_id:
    ...
    if is_paged(request):
        ...
        items, total = await fetch_page(db.hr_attendance, q, page, page_size, sort_field="date", sort_dir=-1)
