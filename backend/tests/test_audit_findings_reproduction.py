...
def test_D1_pagar_menuduh_saat_papan_hilang():
    ...
    # Sesi 4 (2026-09): manajer kini WAJIB punya papan ini (BOARD_ROLES = admin+manager) →
    # payload tanpa papan juga dituduh. Yang tetap tidak dituduh: peran di luar BOARD_ROLES.
    g2 = Guard("D1", "test")
    g2.violations, g2.checks = [], 0
    vhk.check_payload(g2, "manager", payload, {"special_order": 2},
                      {"special-orders", "approval-inbox"})
    assert g2.violations, "manajer tanpa papan PO custom TIDAK dituduh — regresi D1 (manager ∈ BOARD_ROLES)"
    g3 = Guard("D1", "test")
    g3.violations, g3.checks = [], 0
    vhk.check_payload(g3, "sales", payload, {"special_order": 2},
                      {"special-orders", "approval-inbox"})
    assert not g3.violations, "tuduhan palsu untuk beranda sales (bukan BOARD_ROLES)"
