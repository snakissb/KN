...
def test_entity_isolation_on_notifications(admin_tok):
    ...
    def entity_set(items):
        # Sesi 4 (2026-09): peringatan KRITIS sengaja tampil lintas konteks dalam batas
        # `allowed_entity_ids` (lihat routers/notifications.py) — bukan kebocoran.
        return {n.get("entity_id") for n in items
                if n.get("entity_id") and n.get("severity") != "critical"}
