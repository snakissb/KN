...
async def reverse_writeoff(return_id: str, actor: str, roll_ids: List[str] = None,
    ...
    # INV-ATOMIC-01 — klaim retur SESUDAH target ditentukan, SEBELUM JE dibalik/roll dipulihkan:
    # dua un-scrap bersamaan tidak boleh sama-sama membalik jurnal write-off.
    from services import atomic_claim as _saga
    await _saga.claim("sales_returns", return_id, "sales_return_reverse_writeoff", actor=actor)
    now = now_iso()
    combos: set = set()
 ...
 "qty_rolls": (1 if rid else None),
    ...
    await db.sales_returns.update_one({"id": return_id}, _saga.finish_set({
        "updated_at": now, "last_writeoff_reversal_at": now, "last_writeoff_reversal_by": actor}))
    ret = await db.sales_returns.find_one({"id": return_id})
    ret.pop("_id", None)
...
async def relocate_return_rolls(return_id: str, to_warehouse_id: str,
    ...
    # INV-ATOMIC-01 — klaim retur sebelum roll/tag/mutasi ditulis; dua pemindahan
    # bersamaan tidak boleh menulis kaki (leg) & mutasi ganda.
    from services import atomic_claim as _saga
    await _saga.claim("sales_returns", return_id, "sales_return_relocate", actor=actor)
    now = now_iso()
    from_ids = {}
    segs = set()
    total_qty = 0.0
    ...
    _upd = _saga.finish_set({"updated_at": now})
    _upd["$push"] = {"relocation_legs": leg}
    await db.sales_returns.update_one({"id": return_id}, _upd)
