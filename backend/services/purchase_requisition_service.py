...
async def reorder_suggestions(entity_id: Optional[str] = None, ctx: Any = None) -> Dict[str, Any]:
    ...
    # T-09 (audit 2026-09) — samakan aturan dengan hilirnya (`rnd_gate.assert_orderable`
    # saat BUAT PR): mode `block` → produk belum-produksi disembunyikan dari saran;
    # `warn` → tetap tampil dengan penanda; `off` → apa adanya. Saringan keras yang
    # mengabaikan mode akan memutus alur yang sah pada mode `warn`.
    from services import rnd_gate
    lc_mode = await rnd_gate.enforcement_mode(entity_id if entity_id and entity_id != "all" else "")
    ...
    for p in products:
        pid = p["id"]
        lc_orderable = rnd_gate.is_orderable(p)
        if lc_mode == "block" and not lc_orderable:
            continue
        ...
        if rop > 0:
            ...
            rows.append({
                ...
                "lifecycle_warning": (lc_mode == "warn" and not lc_orderable),
            })
        elif avg_daily > 0:
