...
async def options(order: Dict[str, Any]) -> Dict[str, Any]:
    ...
    open_pr = next((str(r.get("open_pr_number") or "") for r in (state or {}).get("candidates", [])
                    if r.get("open_pr_number")), "")
    return {
        ...
        # T4 audit training 2026-06 — JEJAK keputusan (lama → baru); memutuskan ulang
        # menambah baris, tidak menimpa.
        "decisions": decision_history(order),
        "options": {
            "interco": {
            ...
            "reorder": {
                ...
                "hint": (f"{open_pr} masih terbuka — memilih Reorder lagi MENEGASKAN PR itu "
                         "(tidak membuat PR kembar)." if open_pr and kurang else ""),
            },
            "wait": {
...
def decision_history(order: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Jejak keputusan lama → baru. Dokumen lama (hanya `fulfillment_decision`)
    dibaca sebagai jejak satu baris supaya riwayatnya tidak hilang."""
    hist = list(order.get("fulfillment_decisions") or [])
    last = order.get("fulfillment_decision")
    if not hist and last:
        hist = [last]
    return hist


async def _record(order: Dict[str, Any], decision: Dict[str, Any]) -> None:
    prev = order.get("fulfillment_decision") or None
    if prev:
        decision["supersedes"] = {"mode": prev.get("mode", ""), "summary": prev.get("summary", ""),
                                  "by": prev.get("by", ""), "at": prev.get("at", "")}
    hist = decision_history(order) + [decision]
    await db.sales_orders.update_one(
        {"id": order["id"]},
        {"$set": {"fulfillment_decision": decision, "fulfillment_decisions": hist,
                  "updated_at": now_iso()},
         "$push": {"status_history": {
...
async def decide(order_id: str, mode: str, actor: Dict[str, Any], *,
    ...
    await _record(order, decision)
    return {"order_id": order["id"], "order_number": order.get("number"),
            "decision": decision, "decisions": decision_history(order) + [decision], **hasil}
...
async def _reorder_supplier(order, kurang, actor, note):
    ...
    from services import restock_service
    # T4 — memutuskan ulang "Reorder" saat PR-nya masih terbuka BUKAN galat: keputusan
    # ditegaskan ulang menunjuk PR yang ada (tidak lahir PR kembar). Baris yang belum
    # ber-PR tetap dibuatkan PR.
    open_prs = [p for p in await restock_service._open_restock_prs(order["id"])
                if p.get("status") in restock_service.OPEN_PR_STATUSES]
